...
export module ChessEngineAPI {
  ...
  export class ChessboardUI {
    ...
    get isWhitePlaying(): boolean {
      return this.chessboard.isWhitePlaying;
    }

    get capturedPieces(): Array<number> {
      return this.chessboard.capturedPieces;
    }

    get check(): boolean {
      return this.chessboard.check;
    }
    get checkMate(): boolean {
      return this.chessboard.checkMate;
    }
    get staleMate(): boolean {
      return this.chessboard.staleMate;
    }
    get ownCheck(): boolean {
      return this.chessboard.ownCheck;
    }
    get ownCheckMate(): boolean {
      return this.chessboard.ownCheckMate;
    }
    ...
    onclick(row: number, col: number): void {
      if (!this.isPieceSelected) this.setSelectedPiece(row, col);
      else {
        ...
        if (this.chessboard.isLegalMove(this.selectedPieceRow, this.selectedPieceCol, row, col)) {
          ...
          setTimeout(() => {
            let answer = new Suggestor(this.chessboard).suggestMove(this.lookahead, this.breadth);
            if (null !== answer) this.move(answer);
          }, 10);
        }
      }
    }
