...
export class CapturedPiecesComponent {
  ...
  capturedPiecesFilenames(): Array<string> {
    let sorted: Array<number> = this.chessboard.capturedPieces.sort((n1, n2) => this.isBefore(n1, n2));
    let filenames: Array<string> = sorted.map(piece => PieceUtils.fileName(piece));
    return filenames;
  }

  isBefore(piece1: number, piece2: number) {
    let materialValue1: number = PieceUtils.materialValue(piece1);
    let materialValue2: number = PieceUtils.materialValue(piece2);
