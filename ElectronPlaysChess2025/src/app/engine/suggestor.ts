...
export class Suggestor {
  ...
  suggestMove(lookahead: number, breadth: number): Move | null {
    const PERFORMANCE_MEASURE_start = window.performance.now();
    const moves: Move[] = this.chessboard.moves.legalMoves;
    try {
      this.findBestAnswerTo(lookahead, breadth);
      const sortedMoves = moves.sort((m1, m2) => m2.value - m1.value);
      ...
      const PERFORMANCE_MEASURE_SUM = PERFORMANCE_MEASURE_stop - PERFORMANCE_MEASURE_start;
  ...
  findBestAnswerTo(level: number, breadth: number): Move {
    ...
    if (level > 1) {
      ...
      while (index < breadth && index < sortedMoves.length) {
        ...
        const captured = this.chessboard.capturedPieces.length;
        ...
        try {
          ...
          const answerToAnswer = this.findBestAnswerTo(level - 1, breadth);

          answer.value = -answerToAnswer.value;
          index++;
        } catch (ex) {
          if (ex instanceof CheckMateException) {
            answer.value = 100000;
            break;
          } else answer.value = 0;
        } finally {
