...
export class Evaluator {
  ...
  public static evaluatePosition(
    ...
    let positionalSumBlack = 0; // white fields in the middle of the chessboard add to this number
    let threatSumWhite = 0; // sum of the material values of the white pieces that are either protected by white pieces (negative threat)
    // or attacked by black pieces (positive threat)
    let threatSumBlack = 0; // sum of the material values of the black pieces that are either protected by black pieces (negative threat)
    // or attacked by white pieces (positive threat)
    let coverageSumWhite = 0; // number of empty fields white pieces may reach unharmed
    let coverageSumBlack = 0; // number of empty fields black pieces may reach unharmed
    ...
    let result =
      ...
      positionalSumWhite +
      threatSumWhite / 5 +
      (coverageSumWhite * valueOfPawn) / 10 -
    ...
    if (!chessboard.whiteKingHasMoved) {
      if (!chessboard.whiteLeftRookHasMoved) result += 2500;
      if (!chessboard.whiteRightRookHasMoved) result += 2500;
    }
    if (!chessboard.blackKingHasMoved) {
      if (!chessboard.blackLeftRookHasMoved) result += -2500;
      if (!chessboard.blackRightRookHasMoved) result += -2500;
    }
